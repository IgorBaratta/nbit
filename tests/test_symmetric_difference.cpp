...
namespace
{
    ...
    TYPED_TEST(BitSetTest, TestSymmetricDifferenceEmpty)
    {
        // Copy empty bitset
        auto set1 = this->set;
        auto set2 = this->set;
    ...
    TYPED_TEST(BitSetTest, TestSymmetricDifferenceRandom)
    {
        ...
        // Test symmetric difference of large random sets with different sizes
        std::set<int> s1(v1.begin(), v1.end());
        std::set<int> s2(v2.begin(), v2.end());
        std::vector<int> v_sdifference;
        std::set_symmetric_difference(s1.begin(), s1.end(), s2.begin(), s2.end(),
                                      std::back_inserter(v_sdifference));
        ...
        EXPECT_EQ(decode, v_sdifference);
